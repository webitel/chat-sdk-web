import type { AckPayload } from './AckPayload';
import type { ConnectedPayload } from './ConnectedPayload';
import type { DisconnectedPayload } from './DisconnectedPayload';
import type { ErrorPayload } from './ErrorPayload';
import type { PingPayload } from './PingPayload';
import type { WsMessage } from './WsMessage';
import type { WsThread } from './WsThread';
...
class EventPayload {
	...
	get connectedEvent(): ConnectedPayload | undefined {
		return this._connectedEvent;
	}
	set connectedEvent(connectedEvent: ConnectedPayload | undefined) {
		this._connectedEvent = connectedEvent;
	}

	get disconnectedEvent(): DisconnectedPayload | undefined {
		return this._disconnectedEvent;
	}
	set disconnectedEvent(disconnectedEvent: DisconnectedPayload | undefined) {
		this._disconnectedEvent = disconnectedEvent;
	}

	get messageEvent(): WsMessage | undefined {
		return this._messageEvent;
	}
	set messageEvent(messageEvent: WsMessage | undefined) {
		this._messageEvent = messageEvent;
	}

	get threadCreatedEvent(): WsThread | undefined {
		return this._threadCreatedEvent;
	}
	set threadCreatedEvent(threadCreatedEvent: WsThread | undefined) {
		this._threadCreatedEvent = threadCreatedEvent;
	}

	get ackEvent(): AckPayload | undefined {
		return this._ackEvent;
	}
	set ackEvent(ackEvent: AckPayload | undefined) {
		this._ackEvent = ackEvent;
	}

	get errorEvent(): ErrorPayload | undefined {
		return this._errorEvent;
	}
	set errorEvent(errorEvent: ErrorPayload | undefined) {
		this._errorEvent = errorEvent;
	}

	get pingEvent(): PingPayload | undefined {
		return this._pingEvent;
	}
	set pingEvent(pingEvent: PingPayload | undefined) {
		this._pingEvent = pingEvent;
	}
